             ...
             for (User user : mentionedUsers)
             {
                 ...
                 if(!selfMember.canInteract(mentionedUser)){
                	 ...
                	 if(!event.getMember().canInteract(mentionedUser)){
                		 ...
                		 event.getTextChannel().sendMessage("Nice try. You can't remove a role from" + mentionedUser.getAsMention() + " they are higher in status than you. Rekt.").queue();
                		 ...
                		 if(role.contains(p)){
                			...
                			System.out.println(haha);
     						...
     						if(!event.getGuild().getRoles().stream().anyMatch(r -> r.getName().equalsIgnoreCase(haha)))
                     		 ...
                     		 event.getGuild().getController().removeRolesFromMember(mentionedUser, roleset).queue();
                     		event.getChannel().sendMessage("Removed role " + roleset.getAsMention() + " from user " + mentionedUser.getAsMention()).queue();
